...
def index(request):
    return render(request, 'index.html',
                  {'categories': Category.objects.all(),
                   'posts': Post.objects.filter(is_top=True, verified=True, closed=False),
...
def new_post(request):
    if request.method == 'POST':
        ...
        post.currency = 0#post.currency = int(request.POST["currency"][0])
        post.owner_id = request.user.id

        post.rooms = int(request.POST["rooms_count"][0])
        #post.floor = int(request.POST["floor"][0])
        #post.storeys = int(request.POST["storeys"][0])
