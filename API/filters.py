...
class PostOnlyVisibleFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        parameter = request.GET.get('only_visible', default=False) == 'true'
        if parameter:
            return queryset.filter(closed=False, is_archive=False)
        return queryset


class PostAllUsersFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        parameter = request.GET.get('all_users', default=False) == 'true'
        if not parameter:
